    ...
    public function notify_neworder($params) {
        ...
        if($ret['code'] == 0) {
            ...
            if ($neworder_by_sms && $ret['data']['neworder_sms_tpl'] && $ret['data']['neworder_sms_signname'] && $ret['data']['neworder_sms_phone']) {
                ...
                $obj->SendCode($ret['data']['neworder_sms_phone'], $this->codes, $ret['data']['neworder_sms_tpl'], $ret['data']['neworder_sms_signname']);

            }

            if ($neworder_by_mail && $ret['data']['neworder_mail_tpl_title'] && $ret['data']['neworder_mail_tpl_content']  && $ret['data']['neworder_mail_address']) {

                echo $title = preg_replace_callback('/\$\{(.*?)\}/', function($m){return $this->codes[$m[1]];}, $ret['data']['neworder_mail_tpl_title']);

                echo $content = preg_replace_callback('/\$\{(.*?)\}/', function($m){return $this->codes[$m[1]];}, $ret['data']['neworder_mail_tpl_content']);
    ...
    public function notify_asn($params) {
        ...
        if($ret['code'] == 0) {
            ...
            if ($asn_by_sms && $ret['data']['asn_sms_tpl'] && $ret['data']['asn_sms_signname'] && $this->codes['phone']) {
                ...
                $obj->SendCode($this->codes['phone'], $this->codes, $ret['data']['asn_sms_tpl'], $ret['data']['asn_sms_signname']);
