...
function autoOpen (key, cb) {
    var name = 'servicepro' + key
    if (serviceproData[key]) {
        var times = sessionStorage.getItem(name) | 0
        if (times === 0) sessionStorage.setItem(name, 1);
        else if (times === 1) sessionStorage.setItem(name, 2);
        cb && cb(times);
    }
}
var serviceproAr = [
  ...
  {k: 'chat_daovoice', j: '//widget.daovoice.io/widget/e9333ef8.js', bb: function() {
    ...
    daovoice('update');
    autoOpen('autoopen_chat_daovoice', function(times) {
        times === 0 && daovoice('openMessages');
    })
  }},
  {k: 'chat_crisp', j: 'https://client.crisp.chat/l.js', bb: function() {
    ...
    autoOpen('autoopen_chat_crisp', function(times) {
        if (times === 0) $crisp.push(['do', 'chat:open'])
        else if (times === 1) $crisp.push(['do', 'chat:close'])
    })
  }},
  {k: 'chat_qqyzf', j: 'https://yzf.qq.com/xv/web/static/chat_sdk/yzf_chat.min.js', cb: function() {
    ...
    autoOpen('autoopen_chat_qqyzf', function(times) {
        times === 0 && $('.main-contact').click();
    })
  }},
];
